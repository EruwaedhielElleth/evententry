...
async def fetch_logs(gid, limit=50):
    async with pool.acquire() as conn:
...
async def get_state(gid):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT name, closed, user_name, timestamp FROM states WHERE guild_id=$1",
            gid
        )
...
async def build_embed(gid):
    ...
    for b in BUTTONS:
        info = state[b]
        if info["closed"]:
            status += f"🔴 {b} — {info['user']} — {fmt(info['timestamp'])}\n"
        else:
            status += f"🟢 {b} — OPEN\n"
...
class ToggleButton(discord.ui.Button):
    ...
    async def callback(self, interaction):
        ...
        if not info["closed"]:
            await set_state(gid, self.name, True, interaction.user.display_name, now_utc().isoformat(), None)
...
class ViewLogs(discord.ui.Button):
    ...
    async def callback(self, interaction):
        logs = await fetch_logs(interaction.guild.id, 50)
...
async def update_panel(guild):
    ...
    await msg.edit(embed=await build_embed(guild.id), view=PanelView(await get_state(guild.id)))
...
async def setup_panel(interaction):
